...
export const GenericTable = <T,>({ columns, data, refetch, totalCountAndLimit, currentPage, loading, headerClassNames = {}, cellClassNames = {}, basePath = "" }: GenericTableProps<T>) => {
    ...
    const displayedRowsCount = Math.min(currentPageState * pagination.pageSize, totalCountAndLimit.totalCount || 0);
                ...
                <>
                                ...
                                {table.getHeaderGroups().map((headerGroup: any) => (
                                    <TableRow key={headerGroup.id} className="py-1 leading-none">
                                        {headerGroup.headers.map((header: any) => (
                                            <TableHead key={header.id} className={`h-[51px] pl-4 text-left leading-none ${headerClassNames[header.column.id] || ""}`}>
                                                {header.isPlaceholder ? null : flexRender(header.column.columnDef.header as React.ReactNode, header.getContext())}
                                            </TableHead>
                                        ))}
                                    </TableRow>
                                ))}
                                    ...
                                    table.getRowModel().rows.map((row: any) => (
                                        <TableRow key={row.id} data-state={row.getIsSelected() && "selected"} className="h-12 leading-none">
                                            {row.getVisibleCells().map((cell: any) => (
                                                <TableCell key={cell.id} className={`py-1 leading-none ${cellClassNames[cell.column.id] || ""}`}>
                    ...
                    <div className="flex flex-col items-center justify-center space-x-3 py-4 lg:flex-row lg:items-center lg:justify-between lg:space-x-3 lg:py-4">
                        ...
                        <div className="mb-4 flex items-center pt-4 md:justify-end md:pt-0">
                            <GenericPagination currentPage={currentPageState} totalPage={totalPages} onPageChange={onPageChange} basePath={basePath} />
                        </div>
                    </div>
                </>
            )}
