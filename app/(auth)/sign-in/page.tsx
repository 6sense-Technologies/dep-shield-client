...
const SignIn = () => {
    ...
    const BasicSignInMutation = useMutation({
        mutationFn: async (data: TBasicSignInFormInputs) => {
            ...
            return { result, email: data.emailAddress };
        },
        onSuccess: ({ email }) => {
    ...
    if (session.status === "authenticated") {
        ...
        } else if (session.data?.isVerified && session.status === "authenticated") {
            router.push("/dashboard");
            return <Loader />;
        }
    }
                                ...
                                <div className="mt-[4px]">
                                    ...
                                    <button type="button" onClick={handlePasswordVisibility} className="absolute right-5 top-2.5">
                                        {passwordVisible ? (
                                            ...
                                            <EyeOff size={20} className="text-xl text-inputFooterColor" />
                                        )}
                                    </button>
                                </div>
                            </div>
